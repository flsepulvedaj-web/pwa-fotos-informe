...
export async function addCostosFactura({ id, obraId, tipo, item = '', descripcion = '', numeroFactura = '', fecha, tc = 1, avanceNetoPeriodo = 0, anticipoPeriodo = 0, retencionPeriodo = 0, reajustePeriodo = 0, items = [], sourceFileId = '', sourceFileName = '', updatedAt }) {
  ...
  const factura = {
    ...
    retencionPeriodo,
    reajustePeriodo,
    // Desglose de partidas de ESTE período (leído de la hoja DETALLE del
    // Excel, ver costosEstadoPagoParser.js) — [{categoria, item,
    // descripcion, unidad, avanceActualPercent, totalActual}]. No lo llena
    // el formulario a mano, solo la carga por Excel.
    items,
