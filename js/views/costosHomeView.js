...
/**
 * Pantalla de inicio del módulo Costos: lista de obras (las mismas que usan
 * Protocolos y Control — misma tabla `obras`, mismo id). Cada obra tiene su
 * propio presupuesto, modificaciones, facturación y reembolsos.
 */
export async function renderCostosHomeView(container) {
  let obras = await getAllObras();

  function paint() {
    container.innerHTML = `
      <header class="app-header">
        <button class="icon-btn" id="btn-back-home" title="Volver">←</button>
        <span class="header-title">Costos</span>
      </header>
      <main class="view-content">
        ...
        ` : `
          <div class="empty-state">
            ...
            <p>Crea una para empezar a llevar su presupuesto.</p>
          </div>
        `}
      </main>
      <div class="fab-row">
    ...
    container.querySelectorAll('.obra-tile').forEach((tile) => {
      tile.addEventListener('click', () => navigate(`/costos/obra/${tile.dataset.obraId}`));
    });
